...
const AdminDocumentDeliveryPage = () => {
  ...
  // Send documents for selected review stages
  const sendDocumentsForOrder = async (order: Order, reviewStages: string[]) => {
    ...
    try {
      ...
      const emailData = {
        ...
        review_stages: reviewStages.map(stage => stage.replace('_', ' ').toUpperCase()).join(', '),
        documents_count: formattedDocuments.length,
        current_date: new Date().toLocaleDateString(),
        access_expires: 'Never (lifetime access)',
        support_email: 'mohanselemophile@gmail.com'
      };
      ...
      // await sendDocumentDeliveryEmail(emailData);

      console.log('Document delivery email would be sent with data:', emailData);
    ...
    <AdminLayout>
      <div className="px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6">
            <div className="flex flex-col">
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Selected Orders</p>
              <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-200">{selectedOrders.length}</h3>
            </div>
          </div>
        </div>
            ...
            <div className="flex space-x-2">
              ...
              <button
                ...
                className={`inline-flex items-center px-3 py-2 border rounded-md text-sm font-medium ${
                  selectedOrders.length > 0 && !isSending
